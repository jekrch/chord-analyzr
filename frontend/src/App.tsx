...
function App() {
    ...
    const handleChordClick = useCallback((chordNoteNames: string, chordIndex?: number) => {
        ...
        // Update active chord for pattern system - DON'T affect timing
        if (chordIndex !== undefined) {
            setActiveChordIndex(chordIndex);
        } else {
            // If clicking from chord table, reset to no specific chord
                    ...
                    <div className="max-w-7xl mx-auto px-4 py-3">
                        <div className="flex items-center justify-between mb-2">
                            <div className="text-xs text-gray-400 font-medium uppercase tracking-wide">
                                Chord Sequence {activeChordIndex !== null && `• Playing: ${addedChords[activeChordIndex]?.name}`}
                            </div>
